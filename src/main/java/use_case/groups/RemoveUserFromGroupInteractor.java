...
public class RemoveUserFromGroupInteractor implements RemoveUserInputBoundary {
    ...
    public void execute(RemoveUserInputData inputData) {
        try {
            ...
            if (chat.getParticipantUserIds().size() <= 1) {
                outputBoundary.prepareFailView("Cannot remove the last participant from the chat");
                return;
            }
